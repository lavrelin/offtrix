...
Команды: /catalog, /search, /addtocatalog, /review, /catalogpriority, /addcatalogreklama, /catalogviews
"""
import logging
import re
...
async def search_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Если есть аргументы - ищем по тегам/ключевым словам
    if context.args:
        query = ' '.join(context.args)
        context.user_data['search_query'] = query
        posts = await catalog_service.search_posts(query)
        ...
        context.user_data['search_results'] = posts
        context.user_data['search_index'] = 0
        
        # Показываем первый результат
        await send_search_result(update, context, posts[0])
        
        keyboard = []
        if len(posts) > 1:
            keyboard.append([InlineKeyboardButton(f"➡️ Следующие", callback_data="search:next")])
        keyboard.append([InlineKeyboardButton("⬅️ Назад", callback_data="menu:back")])
        
        await update.message.reply_text(
            f"🔍 Найдено результатов: {len(posts)}",
            reply_markup=InlineKeyboardMarkup(keyboard)
        )
        return
    
    # Если нет аргументов - показываем выбор категорий
    keyboard = []
    ...
    text = (
        "🕵🏼‍♀️ **ПОИСК В КАТАЛОГЕ**\n\n"
        "Выберите категорию или введите /search [ключевое слово]:\n\n"
        "Пример: `/search маникюр`"
    )
...
async def addtocatalog_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['catalog_add'] = {'step': 'link', 'media_from_telegram': []}
    ...
    text = (
        ...
        "💡 Медиа будут импортированы автоматически"
    )
...
async def review_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data['catalog_review'] = {
        'post_id': post_id,
        'post_name': post.get('name', 'Специалист'),
        'waiting': True
    }
    ...
    await update.message.reply_text(
        ...
        f"📌 {post.get('name', 'Услуга')}\n\n"
        f"📝 Ваш отзыв (макс. 500 символов):\n\n"
        f"💬 Напишите свое мнение о качестве услуги",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode='Markdown'
    )
...
async def catalogviews_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """👁️ Просмотры постов - /catalogviews (админ команда)"""
    if not Config.is_admin(update.effective_user.id):
        return
    
    stats = await catalog_service.get_views_stats()
    ...
    if stats:
        for i, (post_id, views, name) in enumerate(stats[:20], 1):
            text += f"{i}. 📌 {name} - {views} 👁️\n"
    else:
        text += "Данные еще не собраны"
...
async def handle_catalog_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if 'catalog_add' in context.user_data:
        ...
        if step == 'link':
            ...
            # Автоматический импорт медиа из Telegram
            await update.message.reply_text(
                "⏳ Импортирую медиа из Telegram...",
                parse_mode='Markdown'
            )
            
            media_result = await catalog_service.import_media_from_telegram(text)
            if media_result:
                data['media_from_telegram'] = media_result.get('media', [])
                data['telegram_description'] = media_result.get('description', '')
        ...
        # 2️⃣ Название
            ...
            await update.message.reply_text(
                "🚶‍♀️ Шаг 3 из 4\n\n"
                "🏷️ Добавьте теги (через запятую)\n\n"
                "Пример: маникюр, гель, дизайн ногтей"
            )
        ...
        # 3️⃣ После медиа — добавление тегов
            ...
            # Получаем медиа из импорта
            media_list = data.get('media_from_telegram', [])
            ...
            post_id = await catalog_service.add_post(
                ...
                name=data['name'],
                tags=tags,
                media_files=media_list,  # Передаем импортированное медиа
                telegram_description=data.get('telegram_description', '')
            )
            ...
            if post_id:
                ...
                await update.message.reply_text(
                    ...
                    f"📸 Медиафайлов импортировано: {media_count}",
                    parse_mode='Markdown'
                )
            else:
    ...
    # 💬 Отзыв
        ...
        if review_data.get('waiting'):
            ...
            post_name = review_data.get('post_name', 'Услуга')
...
async def send_search_result(update: Update, context: ContextTypes.DEFAULT_TYPE, post: dict):
