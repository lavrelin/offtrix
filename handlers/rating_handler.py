...
# ============= CALLBACK PREFIXES (УНИКАЛЬНЫЕ V8, ОРИГИНАЛЬНЫЕ) =============
RATING_CALLBACKS = {
    'gender': 'rtp_gender',
    'vote': 'rtp_vote',
    'back': 'rtp_back',
...
async def handle_rate_about(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"• https://www.instagram.com/anna?igsh=xxx&utm_source=qr",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode='Markdown'
    )
...
async def send_rating_to_moderation(
    ...
    try:
        ...
        # Отправка в новую АДМИН группу
        moderation_chat_id = Config.ADMIN_GROUP_ID
        if media_type == 'video':
        ...
        rating_data['posts'][post_id]['moderation_group_id'] = moderation_chat_id
...
async def handle_rate_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data_parts[0].startswith('rtp_'):
...
async def handle_rate_moderation_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    query = update.callback_query
    data_parts = query.data.split(":")
    if data_parts[0].startswith('rtm_'):
        action = data_parts[0][4:]
    else:
        action = data_parts[0]
